...
function OrderCard({ order, onReorder }) {
  ...
  return (
    <div className="order-card">
      ...
      <div className="order-card-footer">
        ...
        {order.order_status !== 'CANCELLED' && (
          <button
            ...
            title={
              unavailableCount > 0
                ? `${unavailableCount} item(s) currently unavailable`
                : 'Add all items to cart'
            }
          >
            {unavailableCount > 0
              ? `Reorder (${unavailableCount} unavailable)`
              : 'Reorder'}
          </button>
        )}
      </div>
      ...
      {expanded &&
        order.producers &&
        order.producers.length > 0 && (
          <div className="order-detail">
            {order.producers.map((producer, pi) => {
              const sc =
                PRODUCER_STATUS_COLOUR[
                  producer.status
                ] ?? {
                  bg: '#f3f4f6',
                  text: '#374151',
              ...
              return (
                <div
                  key={pi}
                  className="order-producer-group"
                >
                  <div className="order-producer-header">
                    <span className="order-producer-name">
                      {producer.producer_name}
                    </span>

                    <span
                      className="order-producer-status"
                      style={{
                        background: sc.bg,
                        color: sc.text,
                      }}
            ...
            >
              Payment: card ending •••• · Address stored
              securely
            </div>
          </div>
        )}
...
export default function CustomerDashboardPage() {
  ...
  const handleReorder = (order) => {
    const allItems =
      order.producers?.flatMap((p) => p.items) ?? [];
    ...
    const skipped =
      allItems.length - available.length;
    ...
    if (available.length === 0) {
      ...
      setReorderMsg(
        `${available.length} item(s) added to cart. ${skipped} item(s) skipped — currently unavailable.`
      );
    } else {
      setReorderMsg(
  ...
  return (
    <div className="customer-dashboard">
      ...
      {loading && (
        <p className="customer-dashboard-loading">
          Loading orders…
        </p>
      )}

      {error && (
        <p className="customer-dashboard-error">
          {error}
        </p>
      )}
