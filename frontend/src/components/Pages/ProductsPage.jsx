...
export default function ProductsPage() {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: 1100, margin: '0 auto' }}>
      <h1 style={{ marginBottom: '0.5rem' }}>{pageTitle}</h1>

      {hasFilter && (
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: '1.25rem', alignItems: 'center' }}>
          <span style={{ fontSize: '0.85rem', color: '#6b7280' }}>Filters:</span>
          {categoryQuery && (
            <span style={{
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', fontWeight: 'bold', lineHeight: 1, padding: 0, color: '#374151' }}
            ...
            {products.map(p => (
              <div key={p.id} style={{ border: '1px solid #eee', borderRadius: 12, overflow: 'hidden', background: '#fff', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '1rem', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <p style={{ margin: 0, fontSize: '0.78rem', color: '#9ca3af' }}>{p.producer_name}</p>
