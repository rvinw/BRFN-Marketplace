...
export default function ProductsPage() {
  ...
  const [foodMiles, setFoodMiles] = useState({}); // { [productId]: miles }
  ...
  }, [searchQuery, categoryQuery, organicOnly]);

  // Calculate food miles whenever products or customer postcode changes
  useEffect(() => {
    if (!customerPostcode || products.length === 0) return;
    products.forEach(async (p) => {
      if (!p.producer_postcode || foodMiles[p.id] !== undefined) return;
      const miles = await getFoodMiles(customerPostcode, p.producer_postcode);
      if (miles !== null) {
        setFoodMiles((prev) => ({ ...prev, [p.id]: miles }));
      }
    });
  }, [products, customerPostcode]);
  ...
  const hasFilter = searchQuery || categoryQuery || organicOnly;
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              border: "1px solid",
              background: organicOnly ? "#a3e635" : "#fff",
              borderColor: organicOnly ? "#65a30d" : "#d1d5db",
              color: organicOnly ? "#1a2e05" : "#374151",
            }}
          >
            🌿 Organic only
          </button>
